...
<div id="page-pengaturan" class="page">
  ...
  <div class="set-group">
    ...
    <div class="set-item">
      <div>
        <div class="set-label">Samakan dengan Akun Lain (Kode 3 Digit)</div>
        <div class="set-sub">Buka menu ini di akun yang datanya BELUM update (akun backup), masukkan kode 3 digit akun sumber yang datanya paling baru. Semua catatan di akun ini akan diganti persis sama dengan akun sumber.</div>
      </div>
      <button class="set-action" style="background:var(--gold); color:#000; border:none;" onclick="promptSyncAccountToCode()">SAMAKAN AKUN</button>
    </div>
  </div>
...
// ======================================================================
// BACKUP & SAMAKAN AKUN LAIN — MODE TARIK DATA (PULL)
// Dijalankan dari AKUN KEDUA (akun backup/tujuan yang datanya belum update).
// User masukin kode 3 digit AKUN PERTAMA (sumber data terbaru).
// Karena alasan keamanan (satu akun tidak boleh baca data akun lain secara
// langsung), proses ini dilakukan lewat "permintaan sinkronisasi":
//  1. Akun kedua (yang lagi login) HAPUS dulu semua catatannya sendiri (izin
//     sendiri, aman) supaya nanti bener-bener jadi PERSIS SAMA, bukan numpuk.
//  2. Akun kedua bikin dokumen permintaan di 'sync_requests' yang ditujukan
//     ke akun pertama (toUid).
//  3. Begitu akun pertama online / lagi buka aplikasi (listener real-time),
//     dia otomatis kirim SEMUA catatannya sendiri (izin sendiri, aman) ke
//     akun kedua. Kalau akun pertama lagi online bareng, prosesnya instan.
// ======================================================================
window.promptSyncAccountToCode = async function() {
    if (!currentUser) return;

    // Pastikan akun ini sendiri sudah punya kode 3 digit juga (biar bisa dipakai buat arah sebaliknya)
    try { await window.initTransferAccount(); } catch (e) { /* nggak masalah kalau gagal, lanjut aja */ }

    const myCodeInfo = window.__myTransferCode ? `<div style="font-size:10px; color:var(--text3); margin-top:10px;">Kode akun ini: <b style="color:var(--gold); letter-spacing:2px;">${window.__myTransferCode}</b></div>` : '';

    const { value: sourceCode } = await Swal.fire({
        title: 'Samakan dengan Akun Lain',
        html: `
          <div style="font-size:11px; color:var(--text2); text-align:left; margin-bottom:10px;">Masukkan kode 3 digit AKUN SUMBER (akun yang datanya paling baru / mau ditiru). Semua catatan di akun ini SEKARANG akan dihapus dan diganti persis sama dengan akun sumber.</div>
          <input id="swal-synccode" class="swal2-input" placeholder="Masukkan 3 Angka" maxlength="3" type="number" style="text-align:center; font-family:Plus Jakarta Sans; font-weight:800; font-size:18px; letter-spacing:2px; max-width:100%; width:80%; margin:0 auto; display:block; box-sizing:border-box;">
          ${myCodeInfo}
        `,
        focusConfirm: false,
        showCancelButton: true,
        ...
        background: 'var(--card)', color: 'var(--text)',
        confirmButtonColor: 'var(--gold)', cancelButtonColor: 'var(--bg3)',
        preConfirm: () => document.getElementById('swal-synccode').value
    });

    if (!sourceCode) return;
    if (sourceCode.length !== 3) return Swal.fire({icon:'error', title:'Harus 3 Angka!', background:'var(--card)', color:'var(--text)'});

    Swal.fire({title: 'Mencari Akun Sumber...', background:'var(--card)', color:'var(--text)', didOpen: () => {Swal.showLoading()}});

    let source;
    try {
        source = await window.lookupTransferTarget(sourceCode);
    } catch (e) {
        return Swal.fire({icon:'error', title:'Gagal Mencari Akun', text: e.message, background:'var(--card)', color:'var(--text)'});
    }

    if (!source) {
        return Swal.fire({icon:'error', title:'Gagal', text:'Kode 3 Angka tidak ditemukan! Pastikan akun sumber sudah buka aplikasi dan sudah punya kode.', background:'var(--card)', color:'var(--text)'});
    }
    if (source.uid === currentUser.uid) {
        return Swal.fire({icon:'warning', title:'Gagal', text:'Nggak bisa samakan dengan akun ini sendiri bro!', background:'var(--card)', color:'var(--text)'});
    }

    const res = await Swal.fire({
        title: 'Konfirmasi Akun Sumber',
        html: `
          <div style="text-align:left; background:var(--bg2); border:1px solid var(--border); border-radius:12px; padding:16px; margin-top:8px;">
            <div style="font-size:10px; font-weight:800; color:var(--text3); text-transform:uppercase; margin-bottom:8px;">Data akan ditarik dari:</div>
            <div style="font-size:16px; font-weight:800; color:var(--gold); margin-bottom:4px;">${escapeHTML(source.nama)}</div>
            <div style="font-size:12px; color:var(--text2); font-family:'JetBrains Mono', monospace;">${escapeHTML(source.email)}</div>
            <div style="font-size:10px; color:var(--text3); margin-top:12px;">Kode: <b style="color:var(--text);">${sourceCode}</b></div>
          </div>
          <div style="font-size:11px; color:var(--red2); margin-top:12px;">⚠️ SEMUA catatan yang ada di AKUN INI SEKARANG akan DIHAPUS, lalu diganti persis sama dengan data akun sumber. Tindakan ini tidak bisa dibatalkan.</div>
        `,
        showCancelButton: true,
        confirmButtonText: 'YA, SAMAKAN SEKARANG',
        cancelButtonText: 'Batal',
        background: 'var(--card)', color: 'var(--text)',
    ...
    if (res.isConfirmed) {
        await window.executeSyncAccount(source.uid, sourceCode);
    }
};

// Dipanggil dari akun kedua (target/backup): kosongkan data sendiri lalu ajukan permintaan tarik data
window.executeSyncAccount = async function(sourceUid, sourceCode) {
    Swal.fire({title: 'Menyiapkan...', html: 'Mengosongkan catatan akun ini, mohon tunggu...', background:'var(--card)', color:'var(--text)', didOpen: () => {Swal.showLoading()}});
    try {
        const mySnap = await getDocs(collection(db, 'users', currentUser.uid, 'transactions'));
        const myTx = [];
        mySnap.forEach(d => myTx.push(d.id));

        const CHUNK_SIZE = 400;
        for (let i = 0; i < myTx.length; i += CHUNK_SIZE) {
            const chunk = myTx.slice(i, i + CHUNK_SIZE);
            const batch = writeBatch(db);
            chunk.forEach(id => batch.delete(doc(db, 'users', currentUser.uid, 'transactions', id)));
            await batch.commit();
        }

        Swal.fire({title: 'Mengirim Permintaan...', html: 'Menunggu akun sumber online untuk kirim data...', background:'var(--card)', color:'var(--text)', didOpen: () => {Swal.showLoading()}});

        const reqRef = doc(collection(db, 'sync_requests'));
        await setDoc(reqRef, {
        ...
        window.__pendingSyncReqId = reqRef.id;
        window.__pendingSyncReqUnsub = onSnapshot(reqRef, snap => {
            if (!snap.exists()) return;
            const d = snap.data();
            if (d.status === 'done') {
                if (window.__pendingSyncReqUnsub) { window.__pendingSyncReqUnsub(); window.__pendingSyncReqUnsub = null; }
                Swal.fire({icon:'success', title:'Berhasil Disamakan!', text: `${d.count || 0} catatan berhasil ditarik dari akun sumber.`, background:'var(--card)', color:'var(--text)'});
            }
        });

        Swal.fire({
            icon: 'info', title: 'Permintaan Terkirim',
            html: 'Data otomatis akan tersalin dalam beberapa detik, diproses di server (tidak perlu akun sumber membuka aplikasi).',
            background: 'var(--card)', color: 'var(--text)'
        });
    } catch (e) {
        Swal.fire({icon:'error', title:'Error Sistem', text: "Gagal: " + e.message, background:'var(--card)', color:'var(--text)'});
...
// ======================================================================
// CATATAN: proses "kirim data dari akun sumber" TIDAK lagi dijalankan dari
// browser (client-side), supaya bisa jalan otomatis walaupun akun sumber
// TIDAK PERNAH buka aplikasi sama sekali. Proses ini sekarang ditangani oleh
// Cloud Function `processSyncRequest` (server-side, trigger otomatis saat
// dokumen baru dibuat di collection 'sync_requests'). Lihat file
// functions/index.js yang disertakan terpisah untuk kode & cara deploy-nya.
// ======================================================================
